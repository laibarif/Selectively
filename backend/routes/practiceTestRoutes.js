  ...
  try {
    ...
    const [readingQuestions] = await db.query(`
      (SELECT q.*, e.text AS extract_text 
       ...
       WHERE q.type = "finalized" AND q.category = "Comprehension" 
       ORDER BY RAND() LIMIT 5)
      UNION
      (SELECT q.*, e.text AS extract_text 
       FROM original_readingquestion q 
       LEFT JOIN original_extract e ON q.extract_id = e.id
       WHERE q.type = "finalized" AND q.category = "Vocabulary" 
       ORDER BY RAND() LIMIT 5);
    `);

    // ✅ Fetch Math Questions (3 Fractions, 3 Percentage, 4 Ratio)
    const [mathQuestions] = await db.query(`
      (SELECT * FROM original_maths WHERE type = "finalized" AND category = "Fraction" ORDER BY RAND() LIMIT 3)
      UNION
      (SELECT * FROM original_maths WHERE type = "finalized" AND category = "Percentage" ORDER BY RAND() LIMIT 3)
      UNION
      (SELECT * FROM original_maths WHERE type = "finalized" AND category = "Ratio" ORDER BY RAND() LIMIT 4);
    `);

    // ✅ Fetch Thinking Skills Questions (10 Random)
    const [thinkingSkillsQuestions] = await db.query(`
      SELECT * FROM original_thinkingskillsquestion WHERE type = "finalized" ORDER BY RAND() LIMIT 10;
    `);

    // ✅ Fetch Writing Test (1 Topic)
    const [writingTopic] = await db.query(`
      SELECT * FROM original_writingquestion WHERE type = "finalized" ORDER BY RAND() LIMIT 1;
    `);
